...
def discover_scannable_files(source_dir: str) -> List[Path]:
    ...
    for file_path in source_path.rglob("*"):
        if file_path.is_file():
            ...
            if (
                file_path.suffix.lower() in scannable_extensions
                or file_path.name in scannable_extensions
                or file_name_lower in special_files
                or any(
                    pattern in file_name_lower for pattern in ["dockerfile", "makefile"]
                )
            ):
...
def enhance_run_with_metadata(
    run: Dict[str, Any], source_dir: str, scanned_files: List[Path]
) -> None:
    """
    ...
    run["originalUriBaseIds"] = {"SRCROOT": {"uri": "file:///"}}
    ...
    invocation = {
        ...
        "workingDirectory": {"uri": f"file://{source_path}", "uriBaseId": "SRCROOT"},
        "properties": {
...
def load_scanned_files_from_ash_list(
    ...
    try:
        ...
        print(
            f"Warning: Could not read ASH files list {files_list_path}: {e}",
            file=sys.stderr,
        )
        return []
...
def add_run_automation_details(
    run: Dict[str, Any], category: Optional[str] = None
) -> None:
    """
...
def enhance_tool_metadata(run: Dict[str, Any]) -> None:
    ...
    if "downloadUri" not in driver:
        driver["downloadUri"] = (
            "https://github.com/aws-actions/automated-security-helper-action"
        )
...
def enhance_rules_metadata(run: Dict[str, Any]) -> None:
    ...
    for rule in driver["rules"]:
        ...
        if "security-severity" not in properties:
            properties["security-severity"] = get_security_severity_score(
                rule.get("id", "")
            )
    ...
    existing_rules = {
        rule.get("id"): rule for rule in driver["rules"] if rule.get("id")
    }
        ...
        for result in run["results"]:
            ...
            if rule_id and rule_id not in existing_rules:
                ...
                rule = {
                    "id": rule_id,
                    "name": rule_id,
                    "shortDescription": {"text": f"Security finding: {rule_id}"},
                    "fullDescription": {
...
def get_security_severity_score(rule_id: str) -> str:
    ...
    # Low severity (0.1-3.9)
        ...
        keyword in rule_id_lower
        for keyword in ["low", "info", "warning", "deprecated", "unused"]
    ):
        return "2.0"
...
def validate_github_limits(sarif: Dict[str, Any]) -> bool:
    ...
    if len(sarif["runs"]) > 20:
        print(
            f"Warning: SARIF has {len(sarif['runs'])} runs, " f"GitHub maximum is 20",
            file=sys.stderr,
        )
        return False
    ...
    for i, run in enumerate(sarif["runs"]):
        ...
        if "results" in run and len(run["results"]) > 25000:
            print(
                f"Warning: Run {i + 1} has {len(run['results'])} results, "
                f"GitHub maximum is 25,000",
                file=sys.stderr,
            )
            return False
        ...
        if (
            ...
            if rules_count > 25000:
                print(
                    f"Warning: Run {i + 1} has {rules_count} rules, "
                    f"GitHub maximum is 25,000",
                    file=sys.stderr,
                )
                return False
        ...
        if "artifacts" in run and len(run["artifacts"]) > 25000:
            print(
                f"Warning: Run {i + 1} has {len(run['artifacts'])} artifacts, "
                f"GitHub maximum is 25,000",
                file=sys.stderr,
            )
            return False
...
def enhance_sarif_with_file_coverage(
    ...
    try:
        ...
        print(
            f"Error: Could not write enhanced SARIF to {output_path}: {e}",
            file=sys.stderr,
        )
        return 0
...
def main():
    ...
    if len(sys.argv) < 3:
        print(
            "Usage: python sarif_enhancer.py <sarif_file> <source_directory> "
            "[output_file] [scanned_files_list]",
            file=sys.stderr,
        )
        print("", file=sys.stderr)
        print(
            "Enhances ASH SARIF output with comprehensive file coverage and "
    ...
    if scanned_files_list and not os.path.isfile(scanned_files_list):
        print(
            f"Error: Scanned files list not found: {scanned_files_list}",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if files_count > 0:
        ...
        print(
            f"🎯 CodeQL will now show '{files_count} files scanned' in the Security tab"
        )
    else:
