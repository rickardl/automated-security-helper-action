...
def load_config(config_path: Optional[str] = None) -> Dict[str, Any]:
    ...
    if yaml is None:
        print("PyYAML not available, configuration file loading disabled", file=sys.stderr)
        return {}
...
def validate_config(config: Dict[str, Any]) -> Tuple[bool, List[str]]:
    ...
    if 'filters' in config:
        ...
        if severity_threshold and severity_threshold not in ['critical', 'high', 'medium', 'low']:
            errors.append("Invalid severity_threshold: must be one of critical, high, medium, low")
    ...
    if 'tools' in config:
        for tool_name, tool_config in config['tools'].items():
            if isinstance(tool_config, dict):
                ...
                env_vars[f'ASH_CONFIG_{tool_name.upper()}_ENABLED'] = str(enabled).lower()
                
                # Tool-specific configurations
                for key, value in tool_config.items():
                    if key != 'enabled':
                        if isinstance(value, list):
                            env_vars[f'ASH_CONFIG_{tool_name.upper()}_{key.upper()}'] = ','.join(map(str, value))
                        else:
                            env_vars[f'ASH_CONFIG_{tool_name.upper()}_{key.upper()}'] = str(value)
...
def main():
    ...
    if len(sys.argv) < 2:
        ...
        print("  export-env              - Export configuration as environment variables")
        sys.exit(1)
