...
"""
    ...
    def test_load_scanned_files_from_ash_list(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            (github_dir / "sarif_pr_commenter.py").write_text(
                "# sarif", encoding="utf-8"
            )
            (unit_dir / "test_severity_filter.py").write_text(
                "# test", encoding="utf-8"
            )

            # Test loading
            scanned_files = load_scanned_files_from_ash_list(
                str(files_list_path), str(source_dir)
            )
    ...
    def test_enhance_sarif_with_scanned_files_list(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            files_count = enhance_sarif_with_file_coverage(
                str(sarif_path), str(source_dir), str(output_path), str(files_list_path)
            )
