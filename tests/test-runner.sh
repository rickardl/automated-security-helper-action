...
test_python_syntax() {
    ...
    local python_files=(
        ...
        "tests/unit/test_sarif_enhancement.py"
...
try:
    ...
    from src.utils import sarif_enhancer
...
# Test SARIF enhancement functionality
test_sarif_enhancement() {
    log "Testing SARIF enhancement functionality..."

    cd "${PROJECT_DIR}"

    # Run the SARIF enhancement unit tests
    if python3 tests/unit/test_sarif_enhancement.py -v 2>/dev/null; then
        log_success "SARIF enhancement unit tests passed"
    else
        log_error "SARIF enhancement unit tests failed"
        return 1
    fi

    log_success "All SARIF enhancement tests passed"
}
